...
function addressCanWait(twiml, request) {
    if (!request.body.SpeechResult.toLowerCase().includes('no')) {
...
function addressClue1(twiml, request) {
    if (!request.body.SpeechResult.toLowerCase().includes('coffee')) {
...
function addressClue3(twiml, request) {
    if (!request.body.SpeechResult.toLowerCase().includes('oxygen')) {
        addressClueWrong(twiml);
        return;
    }
...
function addressClue4(twiml, request) {
    if (!request.body.SpeechResult.toLowerCase().includes('silent')) {
        addressClueWrong(twiml);
        return;
    }
